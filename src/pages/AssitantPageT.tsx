      ...
      <div className={`chat-transition flex-1 flex flex-col h-full ${sidebarOpen ? 'md:ml-0' : 'md:ml-20'}`}>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide bg-white p-4 pb-8">
          ...
          {messages.map((message) => (
            <div key={message.id} className={`message-animation ${message.role === 'user' ? 'pl-10' : 'pr-10'}`}>
                ...
                <div className="flex-1">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                        {message.actions.map((action: string) => {
                        const route = activityRoutes[action];
                        return route ? (
                            <Link to={route} key={action}>
